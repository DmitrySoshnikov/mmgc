...
static auto mm =
    MemoryManager::create<SingleFreeListAllocator, MarkSweepGC, 32>();
...
TEST(MemoryManager, writeBarrier) {
  ...
  auto mm = MemoryManager::create<SingleFreeListAllocator, MarkSweepGC, 32>(
      [&](uint32_t address, Value& value) {
        _address = address;
        _value = value;
      });
